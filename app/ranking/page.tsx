...
async function getRanking(category: string, period: 'weekly' | 'monthly' | 'all', limit = 20): Promise<Product[]> {
  // 週間・月間は review_count + updated_atで近似、将来的にはview_countを別途管理
    ...
    <div className="min-h-screen bg-[#F0F0F0]" style={{ fontFamily: 'Meiryo, "Hiragino Kaku Gothic Pro", sans-serif' }}>
      ...
      <div className="max-w-5xl mx-auto px-3 py-4">
        <div className="bg-white border border-[#ddd]">
          <div className="px-4 py-3 border-b border-[#ddd] bg-[#f8f8f8]">
            <h1 className="text-base font-bold text-[#333]">🏆 人気ランキング</h1>
          </div>
          ...
          <div className="divide-y divide-[#f0f0f0]">
            {products.length === 0 ? (
              <p className="p-6 text-sm text-[#999]">データがありません</p>
            ) : (
              products.map((p, i) => (
                <Link key={p.id} href={`/product/${p.id}`} className="flex items-center gap-4 px-4 py-3 hover:bg-[#FFF5EE] transition-colors">
                  ...
                  <div className="w-10 text-center shrink-0">
                    {i < 3 ? (
                      <span style={{ color: medalColors[i], fontSize: 24 }}>{'●'}</span>
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 text-xs text-[#999]">
                      <span className="text-yellow-500">{'★'.repeat(Math.round(p.review_average))}{'☆'.repeat(5 - Math.round(p.review_average))}</span>
                      <span>({p.review_count.toLocaleString()}件)</span>
